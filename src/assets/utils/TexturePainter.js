...
THREE.TexturePainter = function (renderer, camera, mesh) {
	...
	function initialize (src) {
		...
		console.log("material: ", scope.mesh);
	...
	function getDirections(directions, origin) {
		...
		for (var i = 0; i < 4; i++) {
			...
			let x = (i % 2) * sign * cursorUnits;
			let y = ((i + 1) % 2) * sign * cursorUnits;
		...
		for (var j = 0; j < 4; j++) {
			...
			let x = (((j % 3) == 0) ? -1 : 1) * cursorUnits;
			let y = ((j < 2) ? 1 : -1) * cursorUnits;
	...
	function getDrawLocations() {
		...
		console.log("mesh: ", scope.mesh.geometry);
		// var uvs = scope.mesh.geometry.faceVertexUvs[0];
		var uvs = scope.mesh.geometry.attributes.uv.array;
