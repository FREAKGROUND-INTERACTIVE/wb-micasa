...
THREE.TexturePainter = function ( renderer, camera, mesh, src ) {
	...
	var initialize = function( src ) {
		...
		canvasCursor.width = canvasCursor.height = 40;
		...
		context.ellipse(
			...
			canvasCursor.width/2 - context.lineWidth/2 - 18, // radiusX
			canvasCursor.height/2 - context.lineWidth/2 - 18, // radiusY
			0, // rotation
			0, // angle start
			Math.PI*2 // angle end
		 );
	...
	function faceDraw( vectors ) {
		...
		scope.ctx.fillStyle = scope.color;//"rgba( 247, 37, 133, 1 )";
